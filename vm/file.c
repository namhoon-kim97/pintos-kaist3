...
void vm_file_init(void) {
  lock_init(&swap_file_lock);
  lock_init(&swap_frame_lock);
}
...
static bool file_backed_swap_out(struct page *page) {
  ...
  lock_acquire(&swap_file_lock);
  // printf("file_length2 : %d, write_bytes : %d\n", file_length(info->file),
...
void *do_mmap(void *addr, size_t length, int writable, struct file *file,
  ...
  while (read_bytes > 0 || zero_bytes > 0) {
    ...
    info->offset = offset;
    if (!vm_alloc_page_with_initializer(VM_FILE, addr, writable, lazy_load_file,
                                        info)) {
      return NULL;
    }
