...
void check_addr(uint64_t *ptr) {
  if (ptr == NULL || is_kernel_vaddr(ptr) ||
      !spt_find_page(&thread_current()->spt, ptr))
    exit(-1);
}
...
void close(int fd) {
  ...
  if (is_root) {
    if (not_exsist_dup_list) {
      ...
      new_root_fd = list_entry(list_begin(&root_fd->dup_list),
...
void *mmap(void *addr, size_t length, int writable, int fd, off_t offset) {
  ...
  if (!file_descriptor || length == 0 || file_descriptor->_stdin ||
      file_descriptor->_stdout || pg_ofs(addr) != 0 || pg_ofs(offset) != 0)
    return NULL;
