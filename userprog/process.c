...
static void __do_fork(void *aux) {
  ...
  for (e = list_begin(&parent->fd_list); e != list_end(&parent->fd_list);
    ...
    if (!list_empty(&parent_fd->dup_list)) {
      for (ed = list_begin(&parent_fd->dup_list);
           ed != list_end(&parent_fd->dup_list); ed = list_next(ed)) {
...
static bool lazy_load_segment(struct page *page, void *aux) {
  ...
  /* Load this page. */
  if (file_read(info->file, kpage, info->page_read_bytes) !=
...
void duplicate_fd(struct file_descriptor *new_fd,
                  struct file_descriptor *old_fd, int newfd) {
...
struct file_descriptor *get_fd(int fd, struct file_descriptor **root) {
  ...
  for (e = list_begin(&curr->fd_list); e != list_end(&curr->fd_list);
    ...
    if (!list_empty(&t->dup_list)) {
      for (ed = list_begin(&t->dup_list); ed != list_end(&t->dup_list);
           ed = list_next(ed)) {
